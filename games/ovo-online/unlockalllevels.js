...
function siteLock(urls) {
  ...
  return urls.some((url) => {
    const [allowedHostname, ...allowedPathParts] = url.split("/");
    const allowedPath = allowedPathParts.join("/");

    return currentHostnameParts.some(
      (allowedHostnamePart) =>
        allowedHostnamePart === allowedHostname &&
        currentPathname.startsWith(`/${allowedPath}`)
    );
  });
}
