function my4399unityFramework(Module) {
  ...
  function _Dll_Authentication() {
    console.log("Auth");
    Auth();
  }

  function _Dll_ConsoleLog(text) {
    console.log(UTF8ToString(text));
  }

  function _Dll_GetDeviceInfo() {
    // deviceInfo = sdk.deviceInfo;
    var yandexDeviceInfo = {
    ...
    MyGameInstance.SendMessage(
      "YandexUtilityCallbacks",
      "ReciveDeviceInfoCallback",
      JSON.stringify(yandexDeviceInfo)
    );
  }

  function _Dll_Load() {
    player.getData().then((d) => {
      if (!!d) {
        const jsData = JSON.stringify(d);
        MyGameInstance.SendMessage("YandexSaveSystem", "SetData", jsData);
      } else {
        MyGameInstance.SendMessage("YandexSaveSystem", "SetData", "");
      }
    });
  }

  function _Dll_Save(jsDataUnity) {
    var jsData = UTF8ToString(jsDataUnity);
    var data = JSON.parse(jsData);
    player.setData(data);
  }

  function _Dll_ShowInterstitial() {
    // sdk.adv.showFullscreenAdv({
    //     callbacks: {
    //         onClose: function(wasShown) {
    MyGameInstance.SendMessage("YandexAdsCallbacks", "CloseInterstitial");
    //         },
    //         onError: function(error) {
    //             MyGameInstance.SendMessage("YandexAdsCallbacks", "ErrorInterstitial")
    //         }
    //     }
    // })
  }

  function _Dll_ShowRewarded() {
    // sdk.adv.showRewardedVideo({
    //     callbacks: {
    //         onOpen: () => {
    //             MyGameInstance.SendMessage("YandexAdsCallbacks", "OpenRewarded");
    //             console.log("Video ad open.")
    //         },
    //         onRewarded: () => {
    //             MyGameInstance.SendMessage("YandexAdsCallbacks", "RewardedRewarded");
    //             console.log("Rewarded!")
    //         },
    //         onClose: () => {
    MyGameInstance.SendMessage("YandexAdsCallbacks", "CloseRewarded");
    console.log("Video ad closed.");
    //         },
    //         onError: e => {
    //             MyGameInstance.SendMessage("YandexAdsCallbacks", "ErrorRewarded");
  ...
  function _PlayOneShotWebAudio(
    idAudioSourceComponent,
    fullPath,
    volume,
    loop,
    pitch,
    ...
    idAudioSourceComponent = UTF8ToString(idAudioSourceComponent);
    fullPath = UTF8ToString(fullPath);
    const sound = new Howl({
      ...
      onend: function () {
        var isPlaying =
          sources[idAudioSourceComponent] !== undefined &&
          sources[idAudioSourceComponent].playing();
        var isOneShotPlaying = false;
        if (sourcesPlayOneShot[idAudioSourceComponent]) {
          isOneShotPlaying = sourcesPlayOneShot[idAudioSourceComponent].some(
            function (sound) {
              return sound.playing();
            }
          );
        }
        if (!isOneShotPlaying && !isPlaying) {
          MyGameInstance.SendMessage(
            "WebAudioCallbacks",
            "AudioSourcesFinished",
            idAudioSourceComponent
          );
        }
      },
    ...
    sound.play();
    if (!sourcesPlayOneShot[idAudioSourceComponent])
      sourcesPlayOneShot[idAudioSourceComponent] = [];
    sourcesPlayOneShot[idAudioSourceComponent].push(sound);
  }

  function _PlayWebAudio(
    idAudioSourceComponent,
    fullPath,
    volume,
    loop,
    ...
    idAudioSourceComponent = UTF8ToString(idAudioSourceComponent);
    fullPath = UTF8ToString(fullPath);
    if (sources[idAudioSourceComponent] !== undefined) {
      sources[idAudioSourceComponent].stop();
    }
    const sound = new Howl({
      src: fullPath,
      volume: volume,
      ...
      mute: Boolean(mute),
      onend: function () {
        if (sourcesPlayOneShot[idAudioSourceComponent] !== undefined) {
          var isOneShotPlaying = false;
          if (sourcesPlayOneShot[idAudioSourceComponent]) {
            isOneShotPlaying = sourcesPlayOneShot[idAudioSourceComponent].some(
              function (sound) {
    ...
    sound.play();
    sources[idAudioSourceComponent] = sound;
  }

  function _ReleaseWebAudioSource(idAudioSourceComponent) {
    if (sources[idAudioSourceComponent] !== undefined) {
      this.StopWebAudio(idAudioSourceComponent);
      sources[idAudioSourceComponent] = undefined;
    }
    if (sourcesPlayOneShot[idAudioSourceComponent])
      sourcesPlayOneShot[idAudioSourceComponent] = [];
  ...
  function _SetMasterMute(mute) {
    Howler.mute(Boolean(mute));
  }

  function _SetMasterPause(pause) {
    if (Boolean(pause)) {
      if (sources !== undefined) {
        for (let key in sources) {
          if (sources.hasOwnProperty(key)) {
            let sound = sources[key];
            if (sound.playing()) {
              sound._isMasterPaused = true;
              sound._pausePosition = sound.seek();
              sound.pause();
            }
          }
        }
      }
      if (sourcesPlayOneShot !== undefined) {
        for (let key in sourcesPlayOneShot) {
          if (sourcesPlayOneShot.hasOwnProperty(key)) {
            let values = sourcesPlayOneShot[key];
      ...
      if (sources !== undefined) {
        for (let key in sources) {
          if (sources.hasOwnProperty(key)) {
            let sound = sources[key];
            if (!sound.playing() && sound._isMasterPaused) {
              sound._isMasterPaused = false;
              sound.seek(sound._pausePosition);
              sound.play();
            }
          }
        }
  ...
  function _SetWebAudioLoop(idAudioSourceComponent, loop) {
    idAudioSourceComponent = UTF8ToString(idAudioSourceComponent);
    if (sources[idAudioSourceComponent] !== undefined) {
      sources[idAudioSourceComponent].loop(Boolean(loop));
    }
    if (sourcesPlayOneShot[idAudioSourceComponent] !== undefined) {
      for (const sound of sourcesPlayOneShot[idAudioSourceComponent]) {
        sound.loop(Boolean(loop));
      }
    }
  }

  function _SetWebAudioMute(idAudioSourceComponent, mute) {
    idAudioSourceComponent = UTF8ToString(idAudioSourceComponent);
    if (sources[idAudioSourceComponent] !== undefined) {
      sources[idAudioSourceComponent].mute(Boolean(mute));
    }
    if (sourcesPlayOneShot[idAudioSourceComponent] !== undefined) {
      for (const sound of sourcesPlayOneShot[idAudioSourceComponent]) {
        sound.mute(Boolean(mute));
      }
    }
  }

  function _SetWebAudioPause(idAudioSourceComponent, pause) {
    idAudioSourceComponent = UTF8ToString(idAudioSourceComponent);
    if (Boolean(pause)) {
      if (sources[idAudioSourceComponent] !== undefined) {
        if (sources[idAudioSourceComponent].playing()) {
          sources[idAudioSourceComponent]._pausePosition =
            sources[idAudioSourceComponent].seek();
          sources[idAudioSourceComponent].pause();
        }
      }
      if (sourcesPlayOneShot[idAudioSourceComponent] !== undefined) {
        for (const sound of sourcesPlayOneShot[idAudioSourceComponent]) {
      ...
      if (sources[idAudioSourceComponent] !== undefined) {
        if (!sources[idAudioSourceComponent].playing()) {
          ...
          sources[idAudioSourceComponent].play();
        }
      }
      if (sourcesPlayOneShot[idAudioSourceComponent] !== undefined) {
        for (const sound of sourcesPlayOneShot[idAudioSourceComponent]) {
  ...
  function _SetWebAudioPitch(idAudioSourceComponent, pitch) {
    idAudioSourceComponent = UTF8ToString(idAudioSourceComponent);
    if (sources[idAudioSourceComponent] !== undefined) {
      sources[idAudioSourceComponent].rate(pitch);
    }
    if (sourcesPlayOneShot[idAudioSourceComponent] !== undefined) {
      for (const sound of sourcesPlayOneShot[idAudioSourceComponent]) {
        sound.rate(pitch);
  ...
  function _SetWebAudioVolume(idAudioSourceComponent, volume) {
    idAudioSourceComponent = UTF8ToString(idAudioSourceComponent);
    if (sources[idAudioSourceComponent] !== undefined) {
      sources[idAudioSourceComponent].volume(volume);
    }
    if (sourcesPlayOneShot[idAudioSourceComponent] !== undefined) {
      for (const sound of sourcesPlayOneShot[idAudioSourceComponent]) {
        sound.volume(volume);
  ...
  function _StopWebAudio(idAudioSourceComponent) {
    idAudioSourceComponent = UTF8ToString(idAudioSourceComponent);
    if (sources[idAudioSourceComponent] !== undefined) {
      sources[idAudioSourceComponent].stop();
    }
    if (sourcesPlayOneShot[idAudioSourceComponent] !== undefined) {
      for (var sound of sourcesPlayOneShot[idAudioSourceComponent]) {
        sound.stop();
      }
      if (sourcesPlayOneShot[idAudioSourceComponent])
        sourcesPlayOneShot[idAudioSourceComponent] = [];
  ...
  var asmLibraryArg = {
    Yh: _ConsoleLog,
    tc: _Dll_Authentication,
    hi: _Dll_ConsoleLog,
    uc: _Dll_GetDeviceInfo,
    Ra: _Dll_Load,
    gi: _Dll_Save,
    sc: _Dll_ShowInterstitial,
    rc: _Dll_ShowRewarded,
    Ge: _JS_Accelerometer_IsRunning,
    ob: _JS_Accelerometer_Start,
    nb: _JS_Accelerometer_Stop,
    Ne: _JS_Cursor_SetImage,
    za: _JS_Cursor_SetShow,
    xa: _JS_DOM_MapViewportCoordinateToElementLocalCoordinate,
    gh: _JS_DOM_UnityCanvasSelector,
    ne: _JS_Eval_OpenURL,
    fe: _JS_FileSystem_Initialize,
    _: _JS_FileSystem_Sync,
    Je: _JS_Focus_Window,
    Ee: _JS_GravitySensor_IsRunning,
    kb: _JS_GravitySensor_Start,
    jb: _JS_GravitySensor_Stop,
    Ce: _JS_Gyroscope_IsRunning,
    ib: _JS_Gyroscope_Start,
    hb: _JS_Gyroscope_Stop,
    Fe: _JS_LinearAccelerationSensor_IsRunning,
    mb: _JS_LinearAccelerationSensor_Start,
    lb: _JS_LinearAccelerationSensor_Stop,
    Gc: _JS_Log_Dump,
    ...
    Be: _JS_ScreenOrientation_DeInit,
    ...
    ha: _JS_Sound_Stop,
    aa: _JS_SystemInfo_GetCanvasClientSize,
    Lb: _JS_SystemInfo_GetDocumentURL,
    $a: _JS_SystemInfo_GetGPUInfo,
    ab: _JS_SystemInfo_GetLanguage,
    gb: _JS_SystemInfo_GetMatchWebGLToCanvasSize,
    bb: _JS_SystemInfo_GetMemory,
    cb: _JS_SystemInfo_GetOS,
    eb: _JS_SystemInfo_GetPreferredDevicePixelRatio,
    re: _JS_SystemInfo_GetScreenSize,
    Gb: _JS_SystemInfo_GetStreamingAssetsURL,
    Pe: _JS_SystemInfo_HasAstcHdr,
    db: _JS_SystemInfo_HasCursorLock,
    Ae: _JS_SystemInfo_HasFullscreen,
    ja: _JS_SystemInfo_HasWebGL,
    Oe: _JS_SystemInfo_IsMobile,
    me: _JS_UnityEngineShouldQuit,
    Ve: _JS_Video_CanPlayFormat,
    pf: _JS_Video_Create,
    ga: _JS_Video_Destroy,
    df: _JS_Video_Duration,
    lf: _JS_Video_EnableAudioTrack,
    _e: _JS_Video_GetAudioLanguageCode,
    af: _JS_Video_GetNumAudioTracks,
    ef: _JS_Video_Height,
    hf: _JS_Video_IsPlaying,
    cf: _JS_Video_IsReady,
    Jb: _JS_Video_Pause,
    kf: _JS_Video_Play,
    Ib: _JS_Video_Seek,
    mf: _JS_Video_SetEndedHandler,
    of: _JS_Video_SetErrorHandler,
    bf: _JS_Video_SetLoop,
    Ye: _JS_Video_SetMute,
    gf: _JS_Video_SetPlaybackRate,
    nf: _JS_Video_SetReadyHandler,
    jf: _JS_Video_SetSeekedOnceHandler,
    Ze: _JS_Video_SetVolume,
    We: _JS_Video_Time,
    Xe: _JS_Video_UpdateToTexture,
    ff: _JS_Video_Width,
    ...
    Aa: _JS_WebRequest_Release,
    rf: _JS_WebRequest_Send,
    tf: _JS_WebRequest_SetRedirectLimit,
    sf: _JS_WebRequest_SetRequestHeader,
    uf: _JS_WebRequest_SetTimeout,
    di: _PlayOneShotWebAudio,
    ei: _PlayWebAudio,
    fi: _ReleaseWebAudioSource,
    qc: _SetMasterMute,
    pc: _SetMasterPause,
    oc: _SetMasterVolume,
    Zh: _SetWebAudioLoop,
    ai: _SetWebAudioMute,
    bi: _SetWebAudioPause,
    _h: _SetWebAudioPitch,
    $h: _SetWebAudioVolume,
    ci: _StopWebAudio,
    ...
    Fc: ___cxa_find_matching_catch_4,
    ...
    Lc: ___sys_getcwd,
    Ic: ___sys_getdents64,
    zc: ___sys_getegid32,
    Ac: ___sys_geteuid32,
    Xc: ___sys_getpeername,
    fd: ___sys_getrusage,
    ...
    ue: _emscripten_set_focus_callback_on_thread,
    ve: _emscripten_set_fullscreenchange_callback_on_thread,
    ...
    oh: _glDetachShader,
    nh: _glDisable,
    mh: _glDisableVertexAttribArray,
    jh: _glDrawArrays,
    ...
    Eg: _glGetQueryiv,
    hh: _glGetRenderbufferParameteriv,
    ...
    Fa: _glGetUniformIndices,
    Z: _glGetUniformLocation,
    ic: _glGetUniformiv,
    Uh: _glGetVertexAttribiv,
    Ia: _glInvalidateFramebuffer,
    Oh: _glIsEnabled,
    zg: _glIsVertexArray,
    Tg: _glLinkProgram,
    ...
    E: invoke_viiiiiii,
    O: invoke_viiiiiiii,
    ta: invoke_viiiiiiiii,
    xc: invoke_viiiiiiiiifi,
    Sa: invoke_viiiiiiiiii,
    Bc: invoke_viiiiiiiiiiii,
    yd: invoke_viiij,
    Od: invoke_viiiji,
    Fd: invoke_viij,
    Wd: invoke_viiji,
  ...
  var dynCall_jiii = (Module["dynCall_jiii"] = function () {
    return (dynCall_jiii = Module["dynCall_jiii"] = Module["asm"]["qj"]).apply(
      null,
      arguments
    );
  });
  var dynCall_ijji = (Module["dynCall_ijji"] = function () {
    return (dynCall_ijji = Module["dynCall_ijji"] = Module["asm"]["rj"]).apply(
      null,
      arguments
    );
  });
  var dynCall_vifi = (Module["dynCall_vifi"] = function () {
    return (dynCall_vifi = Module["dynCall_vifi"] = Module["asm"]["sj"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_jdi = (Module["dynCall_jdi"] = function () {
    return (dynCall_jdi = Module["dynCall_jdi"] = Module["asm"]["Dj"]).apply(
      null,
      arguments
    );
  });
  var dynCall_jjji = (Module["dynCall_jjji"] = function () {
    return (dynCall_jjji = Module["dynCall_jjji"] = Module["asm"]["Ej"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_fiii = (Module["dynCall_fiii"] = function () {
    return (dynCall_fiii = Module["dynCall_fiii"] = Module["asm"]["ck"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_iidi = (Module["dynCall_iidi"] = function () {
    return (dynCall_iidi = Module["dynCall_iidi"] = Module["asm"]["lk"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_vidi = (Module["dynCall_vidi"] = function () {
    return (dynCall_vidi = Module["dynCall_vidi"] = Module["asm"]["ok"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_fi = (Module["dynCall_fi"] = function () {
    return (dynCall_fi = Module["dynCall_fi"] = Module["asm"]["tk"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_vfii = (Module["dynCall_vfii"] = function () {
    return (dynCall_vfii = Module["dynCall_vfii"] = Module["asm"]["Lk"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_fffi = (Module["dynCall_fffi"] = function () {
    return (dynCall_fffi = Module["dynCall_fffi"] = Module["asm"]["Rk"]).apply(
      null,
      arguments
    );
  });
  var dynCall_iffi = (Module["dynCall_iffi"] = function () {
    return (dynCall_iffi = Module["dynCall_iffi"] = Module["asm"]["Sk"]).apply(
      null,
      arguments
    );
  });
  var dynCall_ifii = (Module["dynCall_ifii"] = function () {
    return (dynCall_ifii = Module["dynCall_ifii"] = Module["asm"]["Tk"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_fiiiifffi = (Module["dynCall_fiiiifffi"] = function () {
    return (dynCall_fiiiifffi = Module["dynCall_fiiiifffi"] =
  ...
  var dynCall_fiiiffii = (Module["dynCall_fiiiffii"] = function () {
    return (dynCall_fiiiffii = Module["dynCall_fiiiffii"] =
  ...
  var dynCall_ifi = (Module["dynCall_ifi"] = function () {
    return (dynCall_ifi = Module["dynCall_ifi"] = Module["asm"]["wl"]).apply(
      null,
      arguments
    );
  });
  var dynCall_idi = (Module["dynCall_idi"] = function () {
    return (dynCall_idi = Module["dynCall_idi"] = Module["asm"]["xl"]).apply(
      null,
      arguments
    );
  });
  var dynCall_ijii = (Module["dynCall_ijii"] = function () {
    return (dynCall_ijii = Module["dynCall_ijii"] = Module["asm"]["yl"]).apply(
      null,
      arguments
    );
  });
  var dynCall_jji = (Module["dynCall_jji"] = function () {
    return (dynCall_jji = Module["dynCall_jji"] = Module["asm"]["zl"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_jidi = (Module["dynCall_jidi"] = function () {
    return (dynCall_jidi = Module["dynCall_jidi"] = Module["asm"]["Dl"]).apply(
      null,
      arguments
    );
  });
  var dynCall_diji = (Module["dynCall_diji"] = function () {
    return (dynCall_diji = Module["dynCall_diji"] = Module["asm"]["El"]).apply(
      null,
      arguments
    );
  });
  var dynCall_fidi = (Module["dynCall_fidi"] = function () {
    return (dynCall_fidi = Module["dynCall_fidi"] = Module["asm"]["Fl"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_vffi = (Module["dynCall_vffi"] = function () {
    return (dynCall_vffi = Module["dynCall_vffi"] = Module["asm"]["Ml"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_vjii = (Module["dynCall_vjii"] = function () {
    return (dynCall_vjii = Module["dynCall_vjii"] = Module["asm"]["Dm"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_viffffiiiii = (Module["dynCall_viffffiiiii"] = function () {
    return (dynCall_viffffiiiii = Module["dynCall_viffffiiiii"] =
  ...
  var dynCall_vidddfi = (Module["dynCall_vidddfi"] = function () {
    return (dynCall_vidddfi = Module["dynCall_vidddfi"] =
  ...
  var dynCall_jjii = (Module["dynCall_jjii"] = function () {
    return (dynCall_jjii = Module["dynCall_jjii"] = Module["asm"]["Jn"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_jfi = (Module["dynCall_jfi"] = function () {
    return (dynCall_jfi = Module["dynCall_jfi"] = Module["asm"]["Xn"]).apply(
      null,
      arguments
    );
  });
  var dynCall_fji = (Module["dynCall_fji"] = function () {
    return (dynCall_fji = Module["dynCall_fji"] = Module["asm"]["Yn"]).apply(
      null,
      arguments
    );
  });
  var dynCall_fdi = (Module["dynCall_fdi"] = function () {
    return (dynCall_fdi = Module["dynCall_fdi"] = Module["asm"]["Zn"]).apply(
      null,
      arguments
    );
  });
  var dynCall_dji = (Module["dynCall_dji"] = function () {
    return (dynCall_dji = Module["dynCall_dji"] = Module["asm"]["_n"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_dddi = (Module["dynCall_dddi"] = function () {
    return (dynCall_dddi = Module["dynCall_dddi"] = Module["asm"]["lo"]).apply(
      null,
      arguments
    );
  });
  var dynCall_ddi = (Module["dynCall_ddi"] = function () {
    return (dynCall_ddi = Module["dynCall_ddi"] = Module["asm"]["mo"]).apply(
      null,
      arguments
    );
  });
  var dynCall_ddii = (Module["dynCall_ddii"] = function () {
    return (dynCall_ddii = Module["dynCall_ddii"] = Module["asm"]["no"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_jdii = (Module["dynCall_jdii"] = function () {
    return (dynCall_jdii = Module["dynCall_jdii"] = Module["asm"]["vo"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_vdii = (Module["dynCall_vdii"] = function () {
    return (dynCall_vdii = Module["dynCall_vdii"] = Module["asm"]["zo"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_didi = (Module["dynCall_didi"] = function () {
    return (dynCall_didi = Module["dynCall_didi"] = Module["asm"]["Do"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_ijj = (Module["dynCall_ijj"] = function () {
    return (dynCall_ijj = Module["dynCall_ijj"] = Module["asm"]["$o"]).apply(
      null,
      arguments
    );
  });
  var dynCall_vjji = (Module["dynCall_vjji"] = function () {
    return (dynCall_vjji = Module["dynCall_vjji"] = Module["asm"]["ap"]).apply(
      null,
      arguments
    );
  });
  var dynCall_ij = (Module["dynCall_ij"] = function () {
    return (dynCall_ij = Module["dynCall_ij"] = Module["asm"]["bp"]).apply(
      null,
      arguments
    );
  });
  var dynCall_vif = (Module["dynCall_vif"] = function () {
    return (dynCall_vif = Module["dynCall_vif"] = Module["asm"]["cp"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_vid = (Module["dynCall_vid"] = function () {
    return (dynCall_vid = Module["dynCall_vid"] = Module["asm"]["ep"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_iiif = (Module["dynCall_iiif"] = function () {
    return (dynCall_iiif = Module["dynCall_iiif"] = Module["asm"]["ip"]).apply(
      null,
      arguments
    );
  });
  var dynCall_fif = (Module["dynCall_fif"] = function () {
    return (dynCall_fif = Module["dynCall_fif"] = Module["asm"]["jp"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_fiff = (Module["dynCall_fiff"] = function () {
    return (dynCall_fiff = Module["dynCall_fiff"] = Module["asm"]["op"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_fff = (Module["dynCall_fff"] = function () {
    return (dynCall_fff = Module["dynCall_fff"] = Module["asm"]["Xp"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_f = (Module["dynCall_f"] = function () {
    return (dynCall_f = Module["dynCall_f"] = Module["asm"]["$p"]).apply(
      null,
      arguments
    );
  });
  var dynCall_vfff = (Module["dynCall_vfff"] = function () {
    return (dynCall_vfff = Module["dynCall_vfff"] = Module["asm"]["aq"]).apply(
      null,
      arguments
    );
  });
  var dynCall_ff = (Module["dynCall_ff"] = function () {
    return (dynCall_ff = Module["dynCall_ff"] = Module["asm"]["bq"]).apply(
      null,
      arguments
    );
  });
  ...
  var dynCall_fiif = (Module["dynCall_fiif"] = function () {
    return (dynCall_fiif = Module["dynCall_fiif"] = Module["asm"]["dq"]).apply(
      null,
      arguments
    );
  });
