...
class ElementState {
  constructor(a) {
    ...
    this._htmlZIndex = this._actualHtmlIndex = this._wantHtmlIndex = -1;
  }
  SetVisibleFlag(a) {
  ...
  SetWantHTMLIndex(a) {
    this._wantHtmlIndex = a;
  }
  GetActualHTMLIndex() {
    return this._actualHtmlIndex;
  }
  SetActualHTMLIndex(a) {
    this._actualHtmlIndex = a;
  }
  SetHTMLZIndex(a) {
    this._htmlZIndex = a;
  }
...
window.DOMElementHandler = class extends self.DOMHandler {
  ...
  _OnCreate(a) {
    ...
    var e = this._GetFocusElement(c);
    e.addEventListener("focus", (f) => this._OnFocus(b));
    e.addEventListener("blur", (f) => this._OnBlur(b));
    e = a.htmlIndex;
    d.SetWantHTMLIndex(e);
    d.SetHTMLZIndex(a.htmlZIndex);
    this._autoAttach &&
      ((a = this.GetRuntimeInterface().GetAvailableHTMLIndex(e)),
      d.SetActualHTMLIndex(a),
      this.GetRuntimeInterface().GetHTMLWrapElement(a).appendChild(c));
  }
  CreateElement(a, b) {
  ...
  _OnUpdatePosition(a) {
    if (this._autoAttach) {
      var b = this._elementMap.get(a.elementId),
        c = b.GetElement(),
        d = this.GetRuntimeInterface();
      c.style.left = a.left + "px";
      c.style.top = a.top + "px";
      c.style.width = a.width + "px";
      c.style.height = a.height + "px";
      var e = a.fontSize;
      null !== e && (c.style.fontSize = e + "em");
      e = a.htmlIndex;
      b.SetWantHTMLIndex(e);
      e = d.GetAvailableHTMLIndex(e);
      e !== b.GetActualHTMLIndex() &&
        (c.remove(),
        d.GetHTMLWrapElement(e).appendChild(c),
        b.SetActualHTMLIndex(e),
        d._UpdateHTMLElementsZOrder());
      a = a.htmlZIndex;
      a !== b.GetHTMLZIndex() &&
        (b.SetHTMLZIndex(a), d._UpdateHTMLElementsZOrder());
      b.HadFirstUpdate() ||
  ...
  _OnHTMLLayersChanged() {
    if (this._autoAttach)
      for (const b of this._elementMap.values()) {
        const c = this.GetRuntimeInterface().GetAvailableHTMLIndex(
          b.GetWantHTMLIndex()
        );
        var a = b.GetActualHTMLIndex();
        -1 !== c &&
          -1 !== a &&
          c !== a &&
          ((a = b.GetElement()),
          a.remove(),
          this.GetRuntimeInterface().GetHTMLWrapElement(c).appendChild(a),
          b.SetActualHTMLIndex(c));
      }
  }
...
async function CheckSupportsWorkerMode() {
  ...
  try {
    ...
    return console.warn("Error feature detecting worker mode: ", a), !1;
  }
}
...
window.RuntimeInterface = class a {
  constructor(b) {
    ...
    if ("cordova" === this._exportType && this._useWorker && isAndroid) {
    ...
    this.AddRuntimeComponentMessageHandler("canvas", "update-size", (c) =>
      this._OnUpdateCanvasSize(c)
    );
    this.AddRuntimeComponentMessageHandler(
      "canvas",
      "set-html-layer-count",
      (c) => this._OnSetHTMLLayerCount(c)
    );
    this.AddRuntimeComponentMessageHandler(
      "canvas",
      "cleanup-html-layers",
  ...
  Release() {
    ...
    for (const { canvas: b, htmlWrap: c } of this._canvasLayers)
      b.remove(), c.remove();
  ...
  GetAvailableHTMLIndex(b) {
    return Math.min(b, this._canvasLayers.length - 1);
  }
  GetHTMLWrapElement(b) {
    if (0 > b || b >= this._canvasLayers.length)
      throw new RangeError("invalid canvas layer");
    return this._canvasLayers[b].htmlWrap;
  }
  GetRuntimeBaseURL() {
  ...
  GetCssDisplayMode() {
    ...
    const b = this.GetExportType();
    return new Set(["cordova", "nwjs", "macos-wkwebview"]).has(b)
      ? "standalone"
  ...
  async _InitWorker(b, c) {
    ...
    "preview" === this._exportType
      ? ((this._worker = new Worker("previewworker.js", {
        ...
        await new Promise((k, m) => {
          const n = (p) => {
            this._worker.removeEventListener("message", n);
            p.data && "ok" === p.data.type ? k() : m();
          };
    ...
    const f = e.transferControlToOffscreen();
    document.body.appendChild(e);
    const h = document.createElement("div");
    h.className = "c3htmlwrap";
    document.body.appendChild(h);
    this._canvasLayers.push({ canvas: e, htmlWrap: h });
    window.c3canvas = e;
    self.C3_InsertHTMLPlaceholders && self.C3_InsertHTMLPlaceholders();
    let g = b.workerDependencyScripts || [],
      l = b.engineScripts;
    g = await Promise.all(g.map((k) => this._MaybeGetCordovaScriptURL(k)));
    l = await Promise.all(l.map((k) => this._MaybeGetCordovaScriptURL(k)));
    if ("cordova" === this._exportType)
      for (let k = 0, m = b.projectScripts.length; k < m; ++k) {
        const n = b.projectScripts[k],
          p = n[0];
        if (
          p === b.mainProjectScript ||
          "scriptsInEvents.js" === p ||
          p.endsWith("/scriptsInEvents.js")
        )
          n[1] = await this._MaybeGetCordovaScriptURL(p);
      }
    this._worker.postMessage(
      Object.assign(this._GetCommonRuntimeOptions(b), {
        ...
        messagePort: c,
        canvas: f,
        workerDependencyScripts: g,
        engineScripts: l,
        projectScripts: b.projectScripts,
        mainProjectScript: b.mainProjectScript,
        projectScriptsStatus: self.C3_ProjectScriptsStatus,
      }),
      [c, f, ...this._jobScheduler.GetPortTransferables()]
    );
    this._domHandlers = domHandlerClasses.map((k) => new k(this));
    this._FindRuntimeDOMHandler();
    this._runtimeDomHandler._AddDefaultCanvasEventHandlers(e);
    this._runtimeDomHandler._AddDefaultHTMLWrapEventHandlers(h);
    this._runtimeDomHandler._EnableWindowResizeEvent();
    self.c3_callFunction = (k, m) =>
      this._runtimeDomHandler._InvokeFunctionFromJS(k, m);
    "preview" === this._exportType &&
  ...
  async _InitDOM(b, c) {
    ...
    e.className = "c3htmlwrap";
    document.body.appendChild(e);
    this._canvasLayers.push({ canvas: d, htmlWrap: e });
    window.c3canvas = d;
    self.C3_InsertHTMLPlaceholders && self.C3_InsertHTMLPlaceholders();
    this._domHandlers = domHandlerClasses.map((g) => new g(this));
    this._FindRuntimeDOMHandler();
    this._runtimeDomHandler._AddDefaultCanvasEventHandlers(d);
    this._runtimeDomHandler._AddDefaultHTMLWrapEventHandlers(e);
    e = b.engineScripts.map((g) =>
      "string" === typeof g ? new URL(g, this._runtimeBaseUrl).toString() : g
    );
    if (Array.isArray(b.workerDependencyScripts)) {
      var f = [...b.workerDependencyScripts].map((g) =>
        g instanceof Blob ? URL.createObjectURL(g) : g
      );
      e.unshift(...f);
    }
    e = await Promise.all(e.map((g) => this._MaybeGetCordovaScriptURL(g)));
    await Promise.all(e.map((g) => AddScript(g)));
    e = self.C3_ProjectScriptsStatus;
    f = b.mainProjectScript;
    const h = b.projectScripts;
    for (let [g, l] of h)
      if ((l || (l = g), g === f))
        try {
          (l = await this._MaybeGetCordovaScriptURL(l)),
            await AddScript(l),
            "preview" !== this._exportType ||
              e[g] ||
        ...
        (l = await this._MaybeGetCordovaScriptURL(l)), await AddScript(l);
    "preview" === this._exportType &&
  ...
  _OnUpdateCanvasSize(b) {
    if (!this.IsExportingToVideo()) {
      var c = b.styleWidth + "px",
        d = b.styleHeight + "px",
        e = b.marginLeft + "px",
        f = b.marginTop + "px";
      for (const { canvas: h, htmlWrap: g } of this._canvasLayers)
        (h.style.width = c),
          (h.style.height = d),
          (h.style.marginLeft = e),
          (h.style.marginTop = f),
          (g.style.width = c),
          (g.style.height = d),
          (g.style.marginLeft = e),
          (g.style.marginTop = f),
          this._isFirstSizeUpdate &&
            ((h.style.display = ""), (g.style.display = ""));
  ...
  _OnSetHTMLLayerCount(b) {
    var c = b.count,
      d = b.immediate;
    const e = b.styleWidth + "px",
      f = b.styleHeight + "px",
      h = b.marginLeft + "px";
    b = b.marginTop + "px";
    const g = [],
      l = [];
    if (c < this._canvasLayers.length)
      for (; this._canvasLayers.length > c; ) {
        const { canvas: k, htmlWrap: m } = this._canvasLayers.pop();
        m.remove();
        this._useWorker && !d
          ? this._pendingRemoveElements.push(k)
          : k.remove();
      }
    else if (c > this._canvasLayers.length)
      for (let k = 0, m = c - this._canvasLayers.length; k < m; ++k)
        (c = document.createElement("canvas")),
          c.classList.add("c3overlay"),
          this._useWorker
            ? ((d = c.transferControlToOffscreen()), g.push(d), l.push(d))
            : g.push(c),
          document.body.appendChild(c),
          (d = document.createElement("div")),
          d.classList.add("c3htmlwrap", "c3overlay"),
          document.body.appendChild(d),
          (c.style.width = e),
          (c.style.height = f),
          ...
          (d.style.width = e),
          (d.style.height = f),
          (d.style.marginLeft = h),
          (d.style.marginTop = b),
          this._runtimeDomHandler._AddDefaultCanvasEventHandlers(c),
          this._runtimeDomHandler._AddDefaultHTMLWrapEventHandlers(d),
          this._canvasLayers.push({ canvas: c, htmlWrap: d });
    for (const k of this._domHandlers)
      k instanceof window.DOMElementHandler && k._OnHTMLLayersChanged();
    this._UpdateHTMLElementsZOrder();
    return { addedCanvases: g, transferables: l };
  }
  _OnCleanUpHTMLLayers() {
    for (const b of this._pendingRemoveElements) b.remove();
  ...
  _DoUpdateHTMLElementsZOrder() {
    ...
    this._pendingUpdateHTMLZOrder = !1;
    let b = [];
    for (var c of this._domHandlers)
      if (c instanceof window.DOMElementHandler) {
        var d = c._GetAllElementStatesForZOrderUpdate();
        d && b.push(...d);
      }
    b.sort((h, g) => {
      const l = h.GetActualHTMLIndex(),
        k = g.GetActualHTMLIndex();
      if (l !== k) return l - k;
      h = h.GetHTMLZIndex();
      g = g.GetHTMLZIndex();
      return h - g;
    ...
    for (; e < f; ++e) {
      const h = b[e];
      h.GetActualHTMLIndex() !== c &&
        (this._DoUpdateHTMLElementsZOrderOnHTMLLayer(c, b.slice(d, e)),
        (c = h.GetActualHTMLIndex()),
        (d = e));
    }
    d < e && this._DoUpdateHTMLElementsZOrderOnHTMLLayer(c, b.slice(d, e));
  }
  _DoUpdateHTMLElementsZOrderOnHTMLLayer(b, c) {
    if (!(1 >= c.length || b >= this._canvasLayers.length)) {
      c = c.map((l) => l.GetElement());
      var d = new Set(c);
      b = this.GetHTMLWrapElement(b);
      for (
        var e = Array.from(b.children).filter((l) => d.has(l)),
          f = 0,
          h = Math.min(c.length, e.length);
        f < h && c[f] === e[f];
        ++f
      );
      for (var g = f; g < h; ++g) e[g].remove();
      for (g = f; g < h; ++g) b.appendChild(c[g]);
    }
  }
  _GetLocalRuntime() {
  ...
  PostToRuntimeComponentAsync(b, c, d, e, f) {
    const h = nextResponseId++,
      g = new Promise((l, k) => {
        pendingResponsePromises.set(h, { resolve: l, reject: k });
      });
    this._messageChannelPort.postMessage(
      {
        type: "event",
...
function PreventDefaultOnHTMLWrap(a) {
  a.target.tagName &&
    a.target.classList.contains("c3htmlwrap") &&
    a.preventDefault();
}
...
RuntimeInterface$jscomp$1.AddDOMHandlerClass(
  class extends self.DOMHandler {
    constructor(a) {
      ...
      a.AddRuntimeComponentMessageHandler("runtime", "invoke-download", (c) =>
        this._OnInvokeDownload(c)
      );
      a.AddRuntimeComponentMessageHandler("runtime", "load-webfonts", (c) =>
        this._OnLoadWebFonts(c)
      );
      a.AddRuntimeComponentMessageHandler("runtime", "raster-svg-image", (c) =>
        this._OnRasterSvgImage(c)
      );
      a.AddRuntimeComponentMessageHandler(
        "runtime",
        "get-svg-image-size",
        (c) => this._OnGetSvgImageSize(c)
      );
      a.AddRuntimeComponentMessageHandler(
        "runtime",
        "set-target-orientation",
        (c) => this._OnSetTargetOrientation(c)
      );
      a.AddRuntimeComponentMessageHandler("runtime", "register-sw", () =>
        this._OnRegisterSW()
      );
      a.AddRuntimeComponentMessageHandler("runtime", "post-to-debugger", (c) =>
        this._OnPostToDebugger(c)
      );
      a.AddRuntimeComponentMessageHandler("runtime", "go-to-script", (c) =>
        this._OnPostToDebugger(c)
      );
      a.AddRuntimeComponentMessageHandler(
        "runtime",
      ...
      a.AddRuntimeComponentMessageHandler("runtime", "debug-highlight", (c) =>
        this._OnDebugHighlight(c)
      );
      a.AddRuntimeComponentMessageHandler(
        "runtime",
      ...
      a.AddRuntimeComponentMessageHandler("runtime", "add-stylesheet", (c) =>
        this._OnAddStylesheet(c)
      );
      a.AddRuntimeComponentMessageHandler(
        "runtime",
        "script-create-worker",
        (c) => this._OnScriptCreateWorker(c)
      );
      a.AddRuntimeComponentMessageHandler("runtime", "alert", (c) =>
        this._OnAlert(c)
      );
      a.AddRuntimeComponentMessageHandler(
        "runtime",
        "screen-reader-text",
        (c) => this._OnScreenReaderTextEvent(c)
      );
      a.AddRuntimeComponentMessageHandler(
        "runtime",
        "hide-cordova-splash",
      ...
      a.AddRuntimeComponentMessageHandler(
        ...
        (c) => this._SetExportingToVideo(c)
      );
      a.AddRuntimeComponentMessageHandler(
        "runtime",
        "export-to-video-progress",
        (c) => this._OnExportVideoProgress(c)
      );
      a.AddRuntimeComponentMessageHandler("runtime", "exported-to-video", (c) =>
        this._OnExportedToVideo(c)
      );
      a.AddRuntimeComponentMessageHandler(
        "runtime",
        "exported-to-image-sequence",
        (c) => this._OnExportedToImageSequence(c)
      );
      const b = new Set(["input", "textarea", "datalist"]);
      window.addEventListener("contextmenu", (c) => {
        const d = c.target,
          e = d.tagName.toLowerCase();
        b.has(e) || IsInContentEditable(d) || c.preventDefault();
      });
      ...
      window.addEventListener("fullscreenerror", (c) =>
        this._OnFullscreenError(c)
      );
      window.addEventListener("webkitfullscreenerror", (c) =>
        this._OnFullscreenError(c)
      );
      window.addEventListener("mozfullscreenerror", (c) =>
        this._OnFullscreenError(c)
      );
      if (a.IsiOSWebView()) {
        let c = Infinity;
    ...
    _AddDefaultCanvasEventHandlers(a) {
      a.addEventListener("selectstart", PreventDefaultOnCanvasOrDoc);
      a.addEventListener("gesturehold", PreventDefaultOnCanvasOrDoc);
      a.addEventListener("pointerdown", PreventDefaultOnCanvasOrDoc);
    }
    _AddDefaultHTMLWrapEventHandlers(a) {
      a.addEventListener("selectstart", PreventDefaultOnHTMLWrap);
      a.addEventListener("gesturehold", PreventDefaultOnHTMLWrap);
      a.addEventListener("touchstart", PreventDefaultOnHTMLWrap);
    }
    ...
    Attach() {
      ...
      window.addEventListener("mousedown", (b) =>
        this._OnMouseEvent("mousedown", b, DISPATCH_SCRIPT_ONLY)
      );
      window.addEventListener("mousemove", (b) =>
        this._OnMouseEvent("mousemove", b, DISPATCH_SCRIPT_ONLY)
      );
      window.addEventListener("mouseup", (b) =>
        this._OnMouseEvent("mouseup", b, DISPATCH_SCRIPT_ONLY)
      );
      ...
      window.addEventListener("wheel", (b) =>
        this._OnMouseWheelEvent("wheel", b, DISPATCH_RUNTIME_AND_SCRIPT)
      );
  ...
  class extends self.DOMHandler {
    constructor(a) {
      ...
      this.AddRuntimeMessageHandlers([
        ...
        ["set-window-size", (b) => this._OnSetWindowSize(b)],
        ["set-window-position", (b) => this._OnSetWindowPosition(b)],
    ...
    async _OnRequestInstall() {
      ...
      try {
        ...
        return (
          console.error("[Construct] Requesting install failed: ", a),
          { result: "failed" }
        );
      }
    }
    ...
    _OnNavigate(a) {
      ...
      if ("back" === b)
        ...
        self.cordova && self.cordova.InAppBrowser
          ...
          : "preview" === a || this._iRuntime.IsAnyWebView2Wrapper()
    ...
    _OnSetWindowSize(a) {
      window.resizeTo(a.windowWidth, a.windowHeight);
    }
    _OnSetWindowPosition(a) {
      window.moveTo(a.windowX, a.windowY);
    }
  }
);
("use strict");
self.RuntimeInterface.AddDOMHandlerClass(
  class extends self.DOMHandler {
    ...
    async _Init() {
      let a = !1;
      try {
        a = await navigator.storage.persisted();
      } catch (b) {
        (a = !1),
          console.warn(
    ...
    async _OnRequestPersistent() {
      try {
        ...
        return (
          console.error("[Construct] Error requesting persistent storage: ", a),
          { isOk: !1 }
        );
      }
    }
...
self.AudioDOMHandler = class extends self.DOMHandler {
  ...
  *audioInstancesByEffectTag(a) {
      ...
      (this._lastAudioInstance = await this._GetAudioInstance(b, c, d, f, e)),
        k
          ? (this._lastAudioInstance.SetPannerEnabled(!0),
            ...
            k.hasOwnProperty("uid") && this._lastAudioInstance.SetUID(k.uid))
          : "number" === typeof m && 0 !== m
        ...
        this._lastAudioInstance.Play(h, g, l, n);
    } catch (p) {
  ...
  async _AddEffect(a) {
    const b = a.type;
    var c = a.hasOwnProperty("tags") ? a.tags : [a.tag];
    const d = a.params;
    let e;
    if ("convolution" === b)
      try {
  ...
  _SetEffectParam(a) {
    ...
    a = a.time;
    for (const h of b)
      (b = this._effects.get(h.toLowerCase())),
    ...
    if (3 !== b) {
      ...
      if (!f || 1 !== b)
        if (f || 2 !== b) {
          b = null;
          try {
            b = await this._GetAudioInstance(c, d, e, h, f);
          } catch (m) {
...
self.C3AudioInstance = class {
  ...
  SetPan(a, b, c, d, e, f, h) {
    this._isPannerEnabled &&
      ...
      this._pannerConeParams[2] !== h &&
        ((this._pannerConeParams[2] = h),
        (this._pannerNode.coneOuterGain = h)));
  }
  SetPanXYZA(a, b, c, d) {
...
self.C3AudioPhaserFX = class extends AudioFXBase {
  constructor(a, b, c, d, e, f, h) {
    ...
    this._inputNode = this.CreateGain();
    this._dryNode = this.CreateGain();
    this._dryNode.gain.value = 1 - h / 2;
    this._wetNode = this.CreateGain();
    this._wetNode.gain.value = h / 2;
    this._filterNode = this._audioContext.createBiquadFilter();
