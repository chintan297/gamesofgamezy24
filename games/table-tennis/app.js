...
var music;
var audioType = 0;
var muted = !window.GameSnacks.audio.isEnabled();
var splashTimer = 0;
var assetLib;
var preAssetLib;
var isMobile = false;
...
function initStartScreen() {
    ...
    var aButs = window.famobi.hasFeature("credits") ? new Array(oMoreGamesBut, oCreditsBut, oPlayBut) : new Array(oMoreGamesBut, oPlayBut);
    if(!firstRun) {
...
function initMapScreen() {
    window.famobi.showInterstitialAd("button:map", () => {
        _initMapScreen();
    }, "browse");
}
function _initMapScreen() {
    gameState = "map";
    for(var i = 0; i < aMapMarkerPos.length; i++) {
        if(i == oGameData.cupId) {
            var tempX = canvas.width / 2 + aMapMarkerPos[i][0];
...
function initGame(_restart) {
    window.famobi.showInterstitialAd("button:intro:start", () => {
