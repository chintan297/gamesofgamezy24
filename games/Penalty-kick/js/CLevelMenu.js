...
function CLevelMenu() {
	...
	this._createNewLevelPage = function (iStartLevel, iEndLevel) {
		...
		for (var i = iStartLevel; i < iEndLevel; i++) {
			var oBut = new CLevelBut(
				_aPointsX[iCont] + oSprite.width / 2,
				iY + oSprite.height / 2,
				s_aMatches[i],
				_aResults[i],
				i + 1,
				oSprite,
				i + 1 > s_iLastLevel ? false : true,
				oContainerLevelBut,
			)
		...
		if (
			...
			for (var i = 0; i < 31; i++) {
				s_oSpriteLibrary.addSprite(
					"player_" + TEAM_INFO[s_iTeamSelected].player + "_" + i,
					"./sprites/striker/player/player_" +
						TEAM_INFO[s_iTeamSelected].player +
						"_" +
						i +
						".png",
				)
