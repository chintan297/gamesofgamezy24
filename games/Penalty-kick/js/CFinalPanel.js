function CFinalPanel(iLevel, oParentContainer) {
	...
	this._init = function (iLevel) {
		...
		_oResultText = new createjs.Text(
			"YOU 3-2 CPU",
			"80px " + PRIMARY_FONT,
			"#fff",
		)
		...
		_oTotScoreText = new createjs.Text(
			TEXT_TOTAL + " 999999",
			"58px " + PRIMARY_FONT,
			"#fff",
		)
		...
		_oButNext = new CGfxButton(
			CANVAS_WIDTH_HALF + 130,
			CANVAS_HEIGHT_HALF + 190,
			s_oSpriteLibrary.getSprite("but_next"),
			_oContainer,
		)
	...
	this.show = function (szResult, iTotScore, iLevelScore, bLose, bLastLevel) {
		...
		console.log("FinalPanel show called with result ====>", szResult)

		var oSpriteFlag = s_oSpriteLibrary.getSprite(
			"flag_" + TEAM_LABEL[s_aMatches[s_oGame.getCurLevel() - 1]],
		)
		...
		if (bLose) {
			...
			// @DESC:  Trigger GameSnacks gameOver event with score
			if (
				typeof GameSnacks !== "undefined" &&
				GameSnacks.game &&
				GameSnacks.game.gameOver
			) {
				GameSnacks.game.gameOver({ score: iTotScore })
				console.log("GameSnacks gameOver triggered with score: ===>", iTotScore)
			}
		} else {
		...
		console.log("Attempting to show ad after level completion")

		// @DESC:  Show Add On the game after level completion
		if (typeof GameSnacks !== "undefined" && GameSnacks.ad) {
			GameSnacks.ad.showAd({
				type: "interstitial",
				name: "level_complete",
				beforeAd: () => {
					console.log("Ad starting - pausing game update")
					s_oMain.stopUpdate()
				},
				afterAd: () => {
					console.log("Ad completed - resuming game update")
					s_oMain.startUpdate()
				},
				adDismissed: () => {
					console.log("Ad was dismissed - resuming game update")
					s_oMain.startUpdate()
				},
			})
		} else {
	...
	this.centerHomeButton = function () {
		_oButPlayAgain.setVisible(false)
		_oButHome.setX(CANVAS_WIDTH_HALF)
	}
	...
	this._onHome = function () {
		// console.log(
		// 	"Game over testing function on Click Home button Pop up screen complete",
		// )
		// @DESC: Trigger GameSnacks gameOver when exiting from the final panel
		// if (
		// 	typeof GameSnacks !== "undefined" &&
		// 	GameSnacks.game &&
		// 	GameSnacks.game.gameOver
		// ) {
		// 	const score = parseInt(_oTotScoreText.text.replace(/\D/g, "")) || 0
		// 	GameSnacks.game.gameOver({ score })
		// 	console.log(
		// 		"GameSnacks gameOver called from FinalPanel Home button with score:",
		// 		score,
		// 	)
		// }
