class CInterface {
	constructor(iLevel, _iTotScore, oParentContainer) {
		var _aKickIcons
		var _pStartPosAudio
		...
		var _iHighScore = parseInt(getStorageItem("high_score")) || 0
		// var _oHighScoreText

		function setStorageItem(key, value) {
			if (typeof GameSnacks !== "undefined" && GameSnacks.storage?.setItem) {
		...
		function getStorageItem(key) {
			if (typeof GameSnacks !== "undefined" && GameSnacks.storage?.getItem) {
		...
		this._init = function (iLevel) {
			...
			if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
				...
				_pStartPosFullscreen = {
					x: CANVAS_WIDTH - oSprite.width / 2 - 70,
					y: oSprite.height / 2 + 10,
				}
			} else {
				oSprite = s_oSpriteLibrary.getSprite("but_fullscreen")
				_pStartPosFullscreen = {
					x: _pStartPosExit.x - oSprite.width / 2 - 70,
					y: _pStartPosExit.y,
				}
			}
		...
		this.setFinalPanelMultiplayer = function () {
			_oFinalPanel.addEventListener(ON_BACK_MENU, s_oGame.onExit, s_oGame)
			_oFinalPanel.addEventListener(
				ON_RESTART,
				s_oGame.onConfirmRematch,
				s_oGame,
			)
		}
		...
		this.showMultiplierAnim = function (iMult) {
			...
			createjs.Tween.get(_oContainerMult)
				.to({ x: _oScoreText.x }, 250, createjs.Ease.cubicOut)
				.call(function () {
					createjs.Tween.get(_oContainerMult)
						.wait(1000)
						.to({ x: CANVAS_WIDTH + 100 }, 250, createjs.Ease.cubicIn)
						.call(function () {})
				})
		}
		...
		this.refreshScore = function (iTotScore) {
			...
			console.log("Here is our score value =====>" + iTotScore)

			// @DESC: GameSnacks required score update call
			if (typeof GameSnacks !== "undefined" && GameSnacks.score?.update) {
				console.log("GameSnacks Score Update Called:", iTotScore)
				GameSnacks.score.update(iTotScore)
			}

			if (iTotScore > _iHighScore) {
				_iHighScore = iTotScore
				console.log("New High Score:", _iHighScore)
				setStorageItem("high_score", iTotScore)
			}
		}
		...
		this._onConfirmExit = function () {
			// @DESC Trigger GameSnacks gameOver on manual exit
			if (
				typeof GameSnacks !== "undefined" &&
				GameSnacks.game &&
				GameSnacks.game.gameOver
			) {
				const scoreText = _oScoreText.text || ""
				const parsed = parseInt(scoreText.replace(/\D/g, ""))
				const score = isNaN(parsed) ? 0 : parsed

				GameSnacks.game.gameOver({ score })
				console.log(
					"GameSnacks gameOver called on confirm exit with score ====>",
					score,
				)
			} else {
				console.warn("GameSnacks gameOver not available on confirm exit.")
			}
			s_oGame.onExit()
		}
